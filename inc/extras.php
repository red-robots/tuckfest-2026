...
function isTuckFestLandingPage() {
	$lp_bg_image = get_field('landing_page_background_image');
	return ($lp_bg_image) ? $lp_bg_image : false;
}
...
/**
 * Hide the WordPress editor for specific page templates.
 */
add_action('admin_init', 'hide_editor_for_specific_templates');
function hide_editor_for_specific_templates() {
    // 1. Get the Post ID from the URL
    $other_post_id = (isset($_GET['post_ID']) && $_GET['post_ID']) ? $_GET['post_ID'] : '';
    $post_id = (isset($_GET['post']) && $_GET['post']) ? $_GET['post'] : $other_post_id;
    if ( !isset($post_id) || empty($post_id) ) return;

    // 2. Define the template name (e.g., 'templates/my-custom-template.php')
    $template_file = get_post_meta($post_id, '_wp_page_template', true);

    // 3. List of templates where the editor should be hidden
    $hidden_templates = array(
      'templates/page-home2026.php',
    );

    if (in_array($template_file, $hidden_templates)) {
      remove_post_type_support('page', 'editor');
    }
}
